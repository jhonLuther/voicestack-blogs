  ...
  return (
    <React.Fragment>
      {cardType === 'top-image-card' ? (
          ...
          <div className="flex flex-col gap-1 group overflow-hidden">
            ...
            <div className="flex rounded-b-lg p-9 bg-bg-green flex-col items-start gap-10 flex-1">
              <div className="flex flex-col gap-3">
                {post.contentType && (
                  <SubText className="text-white">{post.contentType}</SubText>
                )}
                <H3XL>{post.title}</H3XL>
          ...
          >
            <div className="bg-purple-600 gap-2  text-white px-9 py-11 flex flex-col min-h-60">
              <span className="uppercase text-sm font-medium">
                {post.tags && post?.tags[0]?.tagName}
              </span>
              <h2 className="text-3xl font-bold my-2">{post.title}</h2>
              <p className="text-lg font-normal line-clamp-3 overflow-hidden">
        ...
        <Link href={linkUrl}>
          <div
            className={`flex flex-row gap-4 items-center group hover: transition duration-500 ${className}`}
          >
            {post.mainImage && (
              <ImageLoader
            ...
            <div className="flex flex-col flex-1">
              {post.contentType && <SubText>{post.contentType}</SubText>}
              <H4Large
        ...
        >
          <Link href={linkUrl}>
            <div
              className={`${!isLast && `border-b-2`} pb-6 flex flex-col gap-3 border-b-2 border-gray-800`}
            >
              {post.contentType && <SubText>{post.contentType}</SubText>}
              <H3Large
                className={`group-hover: group-hover:underline underline-offset-4`}
        ...
        <Link href={linkUrl}>
          <div className="flex flex-col gap-1 group hover: transition duration-500 overflow-hidden">
            ...
            <div className="flex py-10 px-9  rounded-b-md bg-cs-purple flex-col items-start gap-10 flex-1">
              <div className="flex flex-col gap-3">
                {post.contentType && <SubText>{post.contentType}</SubText>}
                <h2 className="card-content md:text-5xl text-2xl font-manrope text-white font-extrabold group-hover: group-hover:underline underline-offset-4">
                  {post.title}
                </h2>
        ...
        <Link href={linkUrl}>
          <div
            className={`flex gap-6 items-center justify-center transition-all duration-500 h-full  rounded-lg ${className}`}
          >
            <div className="relative w-48 h-48">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                ...
                >
                  ...
                  <path
                    d="M21.2549 18.8935C21.2549 17.6708 22.5997 16.9255 23.6365 17.5735L37.8221 26.4395C38.7976 27.0492 38.7976 28.4698 37.8221 29.0795L23.6365 37.9455C22.5997 38.5935 21.2549 37.8481 21.2549 36.6255V18.8935Z"
                    fill="white"
                  />
                </svg>
              </div>
        ...
        <div className={`flex flex-col w-full min-h-[250px] group  `}>
          <Link href={linkUrl}>
            {(post.mainImage || post.image) && (
              <div className="overflow-hidden">
                <ImageLoader
                  className="object-center object-cover group-hover:scale-110 transition-transform duration-300 h-full"
                  image={post?.mainImage}
            ...
            <div className="mt-4 flex flex-col gap-1 absolute top-0 left-0 w-full h-full p-8">
              {post.contentType && <SubText>{post.contentType}</SubText>}
              <H4Large
                className={`group-hover: group-hover:underline underline-offset-4`}
              >
