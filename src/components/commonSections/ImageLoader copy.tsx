...
console.log({imageRatio});
  ...
  useEffect(() => {
    ...
    if (useClientWidth) {
      ...
      url = urlForImage(image._id || image, {
        width: cdnImageDimensions.width ,
        height:cdnImageDimensions.height ,
        quality: 90,
      });
    } else {
      url = urlForImage(image._id || image, {
  ...
  if (useClientWidth) {
    return (
      ...
      imageRatio &&
      <div ref={containerRef} className={`relative w-full h-[${(1 / imageRatio) * 100}%] ${className}`}>
        <div
          className={`relative w-full h-[${(1 / imageRatio) * 100}%] flex`}
          style={{
            paddingTop: `${(1 / imageRatio) * 100}%`,
          }}
        >
          {JSON.stringify(imageRatio)}
          <Image
            src={imageUrl}
