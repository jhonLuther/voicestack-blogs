...
const VideoIframe: React.FC<VideoItem> = ({ platform, videoId, title = "" }) => (
  <iframe
...
export const VideoModal: React.FC<VideoProps> = ({
  ...
  return (
    <div onClick={isPopup && onClose}
      className={`${
        isPopup ? "fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50" : ""
      } ${className}`}
    >
      <div
        className={`${
          isPopup ? "relative w-full max-w-xl h-[500px] b rounded-lg overflow-hidden" : "w-full aspect-[16/9] relative"
        }`}
      >
        {isPopup && onClose && (
          <button
